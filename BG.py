...
class Neuro(core.Agent):
    ...
    def prod(self):
        tripla = self.rng.exponential(scale=1/self.eta, size=3)
    ...
    def autofagia(self):
        ...
        if self.stato != "compromesso":
            ...
            self.accumolo_beta -= tripla[1]
            self.accumulo_tau -= tripla[2]
            self.patologia -=  max(tripla[0], tripla[1], tripla[2]) /self.eta
        else:
            self.patologia += max(tripla[0], tripla[1], tripla[2])
    ...
    def to_glia(self, glia):
        self.patologia -= glia.fagocitosi(self.patologia, self.eta)
...
class GliaTest(core.Agent):
    ...
    def step(self, stato:Stato):
        ...
        if self.stato == "DAM1":
            self.da_autofagocitare = np.random.default_rng().exponential(scale=1/ stato.eta)
        ...
        if self.stato == "DAM2":
            ...
            stato.citochine_brain += self.da_fagocitare
            stato.infiammazione_local_b = True
            self.flag = True
            return stato
        
                   
    def fagocitosi(self, patogeno, eta):
